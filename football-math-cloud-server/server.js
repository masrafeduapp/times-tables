...
box.style.cssText="position:fixed;top:12px;right:12px;z-index:99998;background:rgba(7,25,20,.96);color:#fff;border:2px solid #f3c657;border-radius:16px;padding:10px 12px;width:210px;box-shadow:0 8px 28px #0007;font-family:Tahoma,Arial;text-align:center";document.body.appendChild(box);const style=document.createElement("style");style.textContent="#math-timer button{flex:1;border:0;border-radius:8px;padding:7px 3px;font-weight:800;cursor:pointer;background:#f3c657;color:#142018}#math-timer button:active{transform:scale(.97)}#math-timer input:focus{outline:2px solid #f3c657}#math-timer.mt-finished{border-color:#ff5b6e;animation:mtPulse .7s infinite alternate}@keyframes mtPulse{from{transform:scale(1)}to{transform:scale(1.03)}}";document.head.appendChild(style);let total=300,remaining=300,running=false,tick=null;function read(){let m=Math.max(0,Math.min(999,parseInt(document.getElementById("mt-min").value)||0)),s=Math.max(0,Math.min(59,parseInt(document.getElementById("mt-sec").value)||0));document.getElementById("mt-min").value=m;document.getElementById("mt-sec").value=s;return m*60+s}function show(){const m=Math.floor(remaining/60),s=remaining%60;document.getElementById("mt-display").textContent=String(m).padStart(2,"0")+":"+String(s).padStart(2,"0")}function stop(){running=false;if(tick){clearInterval(tick);tick=null}}function finish(){stop();remaining=0;show();box.classList.add("mt-finished");document.getElementById("mt-display").textContent="00:00 🔔";try{const a=new Audio("data:audio/wav;base64,UklGRlIAAABXQVZFZm10IBAAAAABAAEAQB8AAIA+AAACABAAZGF0YQQAAAAA");a.play().catch(()=>{})}catch(e){}}document.getElementById("mt-start").onclick=()=>{if(running)return;if(remaining<=0){total=read();remaining=total;box.classList.remove("mt-finished")}if(remaining<=0)return;running=true;tick=setInterval(()=>{remaining--;show();if(remaining<=0)finish()},1000)};document.getElementById("mt-pause").onclick=stop;document.getElementById("mt-reset").onclick=()=>{stop();box.classList.remove("mt-finished");total=read();remaining=total;show()};["mt-min","mt-sec"].forEach(id=>document.getElementById(id).addEventListener("change",()=>{if(!running){box.classList.remove("mt-finished");total=read();remaining=total;show()}}));show()})()})();</script>`;
}
