...
function bridge(section){
  ...
  return `<script>(function(){const STORE=${JSON.stringify(store)},SECTION=${JSON.stringify(section)};let last='',loading=true;let b=document.getElementById('cloud');if(!b){b=document.createElement('div');b.id='cloud';b.style.cssText='position:fixed;top:10px;left:50%;transform:translateX(-50%);z-index:99999;background:#063d2b;color:white;border:2px solid #e8bd45;border-radius:999px;padding:8px 14px;font-weight:800;box-shadow:0 5px 18px #0006';document.body.appendChild(b)}b.textContent='☁️ جارٍ الاتصال...';const set=(t,e=false)=>{b.textContent=t;b.style.background=e?'#a92635':'#063d2b'};async function get(){const r=await fetch('/api/state?section='+SECTION,{cache:'no-store'});if(!r.ok)throw Error(r.status);return r.json()}async function put(raw){const r=await fetch('/api/state?section='+SECTION,{method:'POST',headers:{'Content-Type':'application/json'},body:raw});if(!r.ok)throw Error(r.status);return r.json()}async function boot(){try{const remote=await get();const rr=remote&&remote.state?JSON.stringify(remote.state):'';const lr=localStorage.getItem(STORE)||'';const lo=lr?JSON.parse(lr):null;const rh=!!(remote&&remote.state);const lh=!!lo;if(rh){if(lr!==rr){localStorage.setItem(STORE,rr);location.reload();return}last=rr}else if(lh){await put(lr);last=lr}else last=lr;set('☁️ متصل بالحفظ السحابي')}catch(e){set('الحفظ المحلي فقط',true)}loading=false;setInterval(sync,900)}async function sync(){if(loading)return;const raw=localStorage.getItem(STORE)||'';if(!raw||raw===last)return;set('☁️ جارٍ الحفظ...');try{await put(raw);last=raw;set('☁️ محفوظ سحابيًا')}catch(e){set('تعذر الحفظ السحابي',true)}}boot()})();</script>`;
}
